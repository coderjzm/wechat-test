<template>
  <view class="container log-list">
    <scroll-view scroll-y style="height:100vh;white-space:nowrap;" :scroll-into-view="toSingerNum" enable-back-to-top
                 @scroll="scroll" scroll-with-animation :scroll-top="scrollTop">
      <view class="list-group" v-for="(group, index) in logs" :key="index">
        <view class="title" :id="group.title">{{group.title}}</view>
        <block v-for="(user, i) in group.items" :key="i">
          <view :id="user.mid" class="list-group-item" @tap='toSinger'>
    ...
    <view class="list-shortcut">
      <block v-for="(item, index) in logs" :key="index">
        <text :class="currentIndex===index?'current':''" :data-id="item.title" @tap='scrollToview'>{{item.title}}
        </text>
      </block>
    </view>
    <!-- 固定在顶部的字母导航 -->
...
<script>
  ...
  export default {
    ...
    onLoad: function () {
      ...
      let list = data.list
      if (code === 0) {
        that.logs = that.normalizeSinger(list)
        wx.hideLoading()
      }
    },
    watch: {
          ...
          if (lHeight !== [0]) {
            ...
            for (let i = 0; i < listHeight.length - 1; i++) {
              ...
              if (newY >= height1 && newY < height2) {
                that.currentIndex = i
                that.fixedTitle = that.logs[i].title
                that.fixedTt(height2 - newY);
                return
              }
            }
            // 当滚动到底部，且-newY大于最后一个元素的上限
            this.currentIndex = listHeight.length - 2
            this.fixedTitle = this.logs[listHeight.length - 2].title
          }
        }, 100)
      },
      fixedTt (newVal) {
